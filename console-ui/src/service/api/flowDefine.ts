...
export async function getDebugInfo(id: number): ResponseResult<FlowDefineInfo> {
  return request.get(`/v1/flow/definition/debugInfo/${id}`);
}

export async function flowDefinePage(params: { appCode: string; pageNum: number; pageSize: number; flowName?: string;
      flowType?: string; flowKey?: string; tagPath?: string }): ResponsePageResult {
